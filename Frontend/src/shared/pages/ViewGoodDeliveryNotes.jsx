...
/** Mock phiếu xuất hàng (GDN) – có thể thay bằng API */
const MOCK_GDN = [
    { gdnid: 1, gdncode: 'GDN-2025-001', releaseRequestCode: 'RR-2025-001', warehouseName: 'Kho chính', issueDate: '2025-02-10', status: 'Draft', submittedAt: null, approvedAt: null },
    { gdnid: 2, gdncode: 'GDN-2025-002', releaseRequestCode: 'RR-2025-002', warehouseName: 'Kho phụ', issueDate: '2025-02-12', status: 'Submitted', submittedAt: '2025-02-12T09:00:00', approvedAt: null },
    { gdnid: 3, gdncode: 'GDN-2025-003', releaseRequestCode: 'RR-2025-003', warehouseName: 'Kho chính', issueDate: '2025-02-14', status: 'Approved', submittedAt: '2025-02-14T10:00:00', approvedAt: '2025-02-14T11:00:00' },
];

const GDN_COLUMNS = [
    { id: 'stt', label: 'STT', getValue: () => '' },
    { id: 'gdncode', label: 'Mã phiếu xuất', getValue: (row) => row.gdncode ?? '' },
    { id: 'releaseRequestCode', label: 'Yêu cầu xuất', getValue: (row) => row.releaseRequestCode ?? '-' },
    { id: 'warehouseName', label: 'Kho xuất', getValue: (row) => row.warehouseName ?? '' },
    { id: 'issueDate', label: 'Ngày xuất', getValue: (row) => row.issueDate ?? '' },
...
const ViewGoodDeliveryNotes = () => {
    ...
    const filteredList = React.useMemo(() => {
        ...
        return list.filter((row) => {
            const matchCode = normalize(row.gdncode).includes(term);
            const matchRR = normalize(row.releaseRequestCode).includes(term);
            const matchWarehouse = normalize(row.warehouseName).includes(term);
            const matchDate = String(row.issueDate || '').toLowerCase().includes(term);
            return matchCode || matchRR || matchWarehouse || matchDate;
        });
    }, [list, searchTerm]);
                ...
                <Card className="list-filter-card" sx={{ mb: 1, borderRadius: 3, border: '1px solid rgba(0,0,0,0.12)', boxShadow: (t) => t.shadows[1] }}>
                    <CardContent sx={{ '&.MuiCardContent-root:last-child': { pb: 2 }, pt: 1, px: 1.5 }}>
                        <Box sx={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 1.5, alignItems: isMobile ? 'stretch' : 'center', flexWrap: 'wrap' }}>
                            <SearchInput
                                placeholder="Tìm theo mã phiếu, yêu cầu xuất, kho, ngày xuất…"
                                value={searchTerm}
                                    ...
                                    <TableHead>
                                        <TableRow>
                                            {visibleColumns.map((col) => (
                                                <TableCell key={col.id} sx={{ ...getColumnCellSx(col.id, getColWidthPct(col.id)), fontWeight: 600, bgcolor: 'grey.50' }} align={col.id === 'actions' ? 'right' : 'left'}>{col.label}</TableCell>
                                            ))}
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                                        {paginatedList.map((row, index) => (
                                            <TableRow key={row.gdnid} hover sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                                {visibleColumns.map((col) => {
                                                    if (col.id === 'stt') return <TableCell key={col.id} align="left" sx={getColumnCellSx(col.id, getColWidthPct(col.id))}>{page * pageSize + index + 1}</TableCell>;
...
export default ViewGoodDeliveryNotes;
