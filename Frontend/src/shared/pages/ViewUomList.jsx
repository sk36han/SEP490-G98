/*
 * Danh sách Đơn vị tính (UoM) – mục con Vật tư.
 */
...
const ViewUomList = () => {
    ...
    const [rows, setRows] = useState(INITIAL_UOMS);
    ...
    const filteredRows = useMemo(() => {
        ...
        return rows.filter(
            (u) => normalize(u.uomCode).includes(term) || normalize(u.uomName).includes(term),
        );
    }, [rows, searchTerm]);
    ...
    const handleResetMock = () => {
        if (!canManage) return;
        setRows(INITIAL_UOMS);
    };

    const handleCreateSubmit = (newUom) => {
        setRows((prev) => [...prev, { uomId: newUom.uomId ?? newUom.id, uomCode: newUom.uomCode ?? newUom.code, uomName: newUom.uomName ?? newUom.name, isActive: true }]);
        setCreateOpen(false);
    };
    ...
    return (
        <Box sx={{ height: '100%', minHeight: 0, minWidth: 0, overflow: 'visible', display: 'flex', flexDirection: 'column', pt: 0, pb: 2, width: '100%', maxWidth: '100%', ml: 0, mr: 0, boxSizing: 'border-box' }}>
                ...
                <Card className="list-filter-card" sx={{ mb: 1, borderRadius: 3, border: '1px solid rgba(0,0,0,0.12)', boxShadow: (t) => t.shadows[1] }}>
                    <CardContent sx={{ '&.MuiCardContent-root:last-child': { pb: 2 }, pt: 1, px: 1.5 }}>
                        <Box sx={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 1.5, alignItems: isMobile ? 'stretch' : 'center', flexWrap: 'wrap' }}>
                            <SearchInput placeholder="Tìm theo mã, tên đơn vị tính..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setPage(0); }} sx={{ flex: '1 1 240px', minWidth: isMobile ? '100%' : 240, maxWidth: isMobile ? '100%' : 520 }} />
                                ...
                                <Table size="small" sx={{ width: '100%', tableLayout: 'fixed' }}>
                                    <TableHead>
                                        <TableRow sx={{ bgcolor: 'grey.50' }}>
                                            ...
                                            <TableCell sx={{ fontWeight: 600, width: '24%' }}>Mã ĐVT</TableCell>
                                            <TableCell sx={{ fontWeight: 600, width: '40%' }}>Tên ĐVT</TableCell>
                                            <TableCell sx={{ fontWeight: 600, width: '20%' }}>Trạng thái</TableCell>
                                        ...
                                        {paginatedRows.length === 0 ? (
                                            <TableRow>
                                                <TableCell colSpan={5} align="center" sx={{ py: 4, color: 'text.secondary' }}>
                                                    {rows.length === 0 ? 'Chưa có đơn vị tính nào.' : 'Không tìm thấy kết quả.'}
                                                </TableCell>
                                            </TableRow>
                                        ) : (
                                            paginatedRows.map((u, index) => (
                                                <TableRow key={u.uomId} hover>
                                                    ...
                                                    <TableCell>
                                                        <Chip label={u.isActive ? 'Hoạt động' : 'Tắt'} size="small" color={u.isActive ? 'success' : 'default'} sx={{ borderRadius: 1.5 }} />
                                                    </TableCell>
            ...
            <CreateUomDialog open={createOpen} onClose={() => setCreateOpen(false)} onSubmit={handleCreateSubmit} />
        </Box>
    );
};
