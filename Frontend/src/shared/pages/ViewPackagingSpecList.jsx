/*
 * Danh sách Quy cách đóng gói – mục con Vật tư.
 */
...
const ViewPackagingSpecList = () => {
    ...
    const handleCreateSubmit = (newItem) => {
        setRows((prev) => [...prev, { packagingSpecId: newItem.packagingSpecId ?? newItem.id, specCode: newItem.specCode, specName: newItem.specName, isActive: true }]);
        setCreateOpen(false);
    };
    ...
    return (
        <Box sx={{ height: '100%', minHeight: 0, minWidth: 0, overflow: 'visible', display: 'flex', flexDirection: 'column', pt: 0, pb: 2, width: '100%', maxWidth: '100%', ml: 0, mr: 0, boxSizing: 'border-box' }}>
                    ...
                    <CardContent sx={{ '&.MuiCardContent-root:last-child': { pb: 2 }, pt: 1, px: 1.5 }}>
                        <Box sx={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 1.5, alignItems: isMobile ? 'stretch' : 'center', flexWrap: 'wrap' }}>
                            ...
                            <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center', ml: isMobile ? 0 : 'auto', flexWrap: 'wrap' }}>
                                ...
                                {canManage && (
                                    <IconButton size="small" onClick={handleResetMock} sx={{ flexShrink: 0 }} title="Khôi phục dữ liệu mẫu">
                                        <RefreshCw size={18} />
                                    </IconButton>
                                )}
                            </Box>
                                        ...
                                        {paginatedRows.length === 0 ? (
                                            <TableRow>
                                                <TableCell colSpan={5} align="center" sx={{ py: 4, color: 'text.secondary' }}>
                                                    {rows.length === 0 ? 'Chưa có quy cách nào.' : 'Không tìm thấy kết quả.'}
                                                </TableCell>
                                            </TableRow>
                                        ) : (
                                            paginatedRows.map((p, index) => (
                                                <TableRow key={p.packagingSpecId} hover>
                                                    ...
                                                    <TableCell align="right">
                                                        {canManage && (
                                                            <IconButton size="small" onClick={() => {}} aria-label="Chỉnh sửa">
                                                                <Edit3 size={18} />
                                                            </IconButton>
                                                        )}
                                                    </TableCell>
            ...
            <CreatePackagingSpecDialog open={createOpen} onClose={() => setCreateOpen(false)} onSubmit={handleCreateSubmit} />
        </Box>
    );
};
