...
import { Search, Edit, Package, Download, Eye, Plus } from 'lucide-react';
...
const ItemList = () => {
    ...
    return (
        <Container maxWidth="xl" sx={{ py: 4 }}>
            ...
            <Paper elevation={3} sx={{ p: 3, borderRadius: 4, background: 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(20px)', boxShadow: '0 8px 32px rgba(0, 0, 0, 0.05)', border: '1px solid rgba(255, 255, 255, 0.3)' }}>
                <Box sx={{ mb: 4, display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'space-between' }}>
                    ...
                    />
                    <Button variant="contained" startIcon={<Plus size={18} />} onClick={() => navigate('/items/create')} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 600 }}>
                        Thêm vật tư
                    </Button>
                    <Button variant="outlined" startIcon={<Download size={18} />} onClick={handleExport} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 600 }}>Xuất Excel</Button>
                </Box>
                    ...
                    <Table stickyHeader size="small">
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell sx={{ fontWeight: 'bold', bgcolor: 'grey.50', color: 'text.secondary', whiteSpace: 'nowrap' }}>Category</TableCell>
                        ...
                        <TableBody>
                            {filteredItems.length === 0 ? (
                                ...
                                filteredItems.map((item, index) => (
                                    <TableRow key={item.itemId} hover sx={{ '&:last-child td, &:last-child th': { border: 0 }, transition: 'background-color 0.2s', '&:hover': { backgroundColor: 'rgba(25, 118, 210, 0.04) !important' } }}>
                                        ...
                                        <TableCell><Chip label={item.isActive ? 'Active' : 'Inactive'} size="small" color={item.isActive ? 'success' : 'default'} variant="filled" sx={{ borderRadius: 1.5 }} /></TableCell>
            ...
            {totalPages > 1 && (
                <Box sx={{ mt: 3, display: 'flex', justifyContent: 'center' }}>
                    <Pagination count={totalPages} page={pageNumber} onChange={(e, page) => setPageNumber(page)} color="primary" showFirstButton showLastButton />
                </Box>
            )}
